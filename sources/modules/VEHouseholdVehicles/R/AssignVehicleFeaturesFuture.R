...
#'
#' \code{AssignVehicleFeaturesFuture} create vehicle table and populate with
...
AssignVehicleFeaturesFuture <- function(L) {
  ...
  FwyLaneMiPC_Bz <- L$Year$Marea$FwyLaneMiPCFuture[match(L$Year$Bzone$Marea, L$Year$Marea$Marea)]*1000 # To match RPAT numbers
  Hh_df$FwyLaneMiPC <- FwyLaneMiPC_Bz[match(L$Year$Household$HhPlaceTypes, L$Year$Bzone$Bzone)]
  TranRevMiPC_Bz <- L$Year$Marea$TranRevMiPCFuture[match(L$Year$Bzone$Marea, L$Year$Marea$Marea)]
  ...
  Out_ls$Year$Vehicle$HhIdFuture <-
    with(L$Year$Household, rep(HhId[Use], Hh_df$Hhvehcnt[Use]))
  attributes(Out_ls$Year$Vehicle$HhIdFuture)$SIZE <-
    max(nchar(Out_ls$Year$Vehicle$HhIdFuture))
  Out_ls$Year$Vehicle$VehIdFuture <-
    with(Hh_df,
         paste(rep(HhId[Use], Hhvehcnt[Use]),
               unlist(sapply(Hhvehcnt[Use], function(x) 1:x)),
               sep = "-"))
  attributes(Out_ls$Year$Vehicle$VehIdFuture)$SIZE <-
    max(nchar(Out_ls$Year$Vehicle$VehIdFuture))
  Out_ls$Year$Vehicle$AzoneFuture <-
    with(L$Year$Household, rep(Azone[Use], Hh_df$Hhvehcnt[Use]))
  attributes(Out_ls$Year$Vehicle$AzoneFuture)$SIZE <-
    max(nchar(Out_ls$Year$Vehicle$AzoneFuture))
  Out_ls$Year$Vehicle$MareaFuture <-
    with(L$Year$Household, rep(Marea[Use], Hh_df$Hhvehcnt[Use]))
  attributes(Out_ls$Year$Vehicle$MareaFuture)$SIZE <-
    max(nchar(Out_ls$Year$Vehicle$MareaFuture))
  Out_ls$Year$Vehicle$TypeFuture <- unlist(Hh_df$VehType[Use])
  Out_ls$Year$Vehicle$TypeFuture[Out_ls$Year$Vehicle$TypeFuture=="LtTruck"] <- "LtTrk"
  attributes(Out_ls$Year$Vehicle$TypeFuture)$SIZE <- max(nchar(Out_ls$Year$Vehicle$TypeFuture))
  Out_ls$Year$Vehicle$AgeFuture <- unlist(Hh_df$VehAge[Use])
  Out_ls$Year$Vehicle$MileageFuture <- unlist(Hh_df$VehMpg[Use])
  Out_ls$Year$Vehicle$DvmtPropFuture <- unlist(Hh_df$DvmtProp[Use])

  Out_ls$Year$Household <-
    list(NumLtTrkFuture = Hh_df$NumLtTruck,
         NumAutoFuture = Hh_df$NumAuto,
         VehiclesFuture = Hh_df$Hhvehcnt)
