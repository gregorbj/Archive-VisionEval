...
The **CreateBzones** module creates a set of simulated Bzones which represent typical census block groups and reflect scenario inputs regarding the proportional allocation of Azone households to three development types: Metropolitan, Town, and Rural. The module creates a "Bzone" table and populates it with several attributes including *Bzone* (a unique ID), *Azone* (the corresponding Azone), *Marea* (the corresponding Marea), *DevType* (the development type), and *NumHh* (the number of households). The model is run in the framework with the following command:
```{r, eval=FALSE}
...
The *CreateBzones* module must be run after the *CreateHouseholds* module is run and before the *CreateBzoneDev* module is run. The module is run for the entire model area at once (the RunBy component is assigned the value "Region"). This is done because the full set of Bzones must be created in order to determine how many rows the *Bzone* table will have. This needs to be determined in order to initialize the table.
...
DevTypeProp_df <- read.csv("../tests/data/devtype_proportions.csv", as.is = TRUE)
